#!/usr/bin/env bash
# Ensure sites and databases exist on the standby (idempotent).

bootstrap_site_on_standby() {
  local domain="$1" site_user="$2" site_type="$3" php_version="$4" vhost_template="$5" site_password="${6:-}"
  local password

  if [[ -n "${site_password}" ]]; then
    password="${site_password}"
  else
    password="$(openssl rand -base64 18 | tr -d '/+=' | head -c 20)"
    log_warn "No panel password for ${site_user}; using temporary password (shadow hash sync will overwrite login hash)"
  fi

  if ! site_type_allowed "${site_type}"; then
    log_warn "Skipping site ${domain}: type '${site_type}' not in BOOTSTRAP_SITE_TYPES"
    return 0
  fi

  local exists
  exists="$(remote "sqlite3 '${CLP_DB_PATH}' \"SELECT id FROM site WHERE domain_name = '$(sql_escape "${domain}")' LIMIT 1;\"")" || true
  if [[ -n "${exists}" ]]; then
    log_info "Standby already has site ${domain} (id=${exists})"
    return 0
  fi

  # Also skip if Linux user already exists without a site row (partial prior run)
  if remote "id -u $(printf '%q' "${site_user}") >/dev/null 2>&1"; then
    log_warn "Linux user ${site_user} exists on standby but site ${domain} missing in panel — create site in UI or clean up user"
  fi

  # Ignore panel password hashes (bcrypt etc.) — clpctl needs a plaintext password.
  if [[ "${password}" == \$* ]]; then
    password="$(openssl rand -base64 18 | tr -d '/+=' | head -c 20)"
    log_warn "Panel DB has a hash for ${site_user}; using a temporary password (shadow sync will match login)"
  fi

  vhost_template="${vhost_template//$'\r'/}"
  vhost_template="${vhost_template#"${vhost_template%%[![:space:]]*}"}"
  vhost_template="${vhost_template%"${vhost_template##*[![:space:]]}"}"
  # Custom nginx stored in the panel DB is not a clpctl template name.
  if [[ -z "${vhost_template}" || "${vhost_template}" == "#"* || "${vhost_template}" == *$'\n'* \
     || "${vhost_template}" == *"server {"* || "${#vhost_template}" -gt 80 ]]; then
    vhost_template="Generic"
  fi

  log_info "Creating ${site_type} site on standby: ${domain} (user=${site_user} template=${vhost_template:-Generic})"

  case "${site_type}" in
    php)
      [[ -n "${php_version}" ]] || php_version="8.3"
      [[ -n "${vhost_template}" ]] || vhost_template="Generic"
      if ! remote clpctl site:add:php \
        --domainName="${domain}" \
        --phpVersion="${php_version}" \
        --vhostTemplate="${vhost_template}" \
        --siteUser="${site_user}" \
        --siteUserPassword="${password}"; then
        if [[ "${vhost_template}" != "Generic" ]]; then
          log_warn "site:add:php failed with template '${vhost_template}'; retrying Generic"
          remote clpctl site:add:php \
            --domainName="${domain}" \
            --phpVersion="${php_version}" \
            --vhostTemplate="Generic" \
            --siteUser="${site_user}" \
            --siteUserPassword="${password}"
        else
          return 1
        fi
      fi
      ;;
    nodejs)
      remote clpctl site:add:nodejs \
        --domainName="${domain}" \
        --siteUser="${site_user}" \
        --siteUserPassword="${password}" \
        --nodejsVersion=22 \
        --appPort=3000
      ;;
    python)
      remote clpctl site:add:python \
        --domainName="${domain}" \
        --siteUser="${site_user}" \
        --siteUserPassword="${password}" \
        --pythonVersion=3.12 \
        --appPort=8080
      ;;
    static)
      remote clpctl site:add:static \
        --domainName="${domain}" \
        --siteUser="${site_user}" \
        --siteUserPassword="${password}"
      ;;
    reverse-proxy)
      remote clpctl site:add:reverse-proxy \
        --domainName="${domain}" \
        --siteUser="${site_user}" \
        --siteUserPassword="${password}" \
        --reverseProxyUrl="http://127.0.0.1:8080"
      ;;
    *)
      log_warn "No bootstrap handler for type=${site_type} domain=${domain}"
      return 0
      ;;
  esac

  log_ok "Created site ${domain} on standby"
}

bootstrap_database_on_standby() {
  local domain="$1" db_name="$2" db_user="$3" site_user="$4"
  local password

  [[ -n "${db_name}" ]] || return 0
  [[ -n "${db_user}" ]] || db_user="${db_name}"

  local exists
  exists="$(remote "sqlite3 '${CLP_DB_PATH}' \"SELECT id FROM database WHERE name = '$(sql_escape "${db_name}")' LIMIT 1;\"")" || true
  if [[ -n "${exists}" ]]; then
    log_info "Standby already has database ${db_name}"
    return 0
  fi

  if password="$(guess_db_password "${site_user}" "${domain}" "${db_name}")"; then
    log_info "Using DB password from app config for ${db_name}"
  else
    password="$(openssl rand -base64 18 | tr -d '/+=' | head -c 24)"
    log_warn "Could not find DB password for ${db_name}; generated one. App configs on standby must match primary (they will after file sync if primary embeds the password)."
    echo "${domain}|${db_name}|${db_user}|${password}" >>"${CLP_SYNC_STATE_DIR}/generated-db-passwords.log"
    chmod 600 "${CLP_SYNC_STATE_DIR}/generated-db-passwords.log"
  fi

  log_info "Creating database ${db_name} for ${domain} on standby"
  if remote clpctl db:add \
    --domainName="${domain}" \
    --databaseName="${db_name}" \
    --databaseUserName="${db_user}" \
    --databaseUserPassword="${password}"; then
    log_ok "Created database ${db_name}"
  else
    log_warn "db:add failed for ${db_name} (may already exist); continuing"
  fi
}

# Create any missing sites + DBs from current inventory snapshot.
run_bootstrap() {
  local db_snap="$1"
  local id domain user type php_ver vhost
  local site_id domain_name site_user db_name db_user

  log_info "Bootstrapping missing sites on standby"
  local site_password
  while IFS='|' read -r id domain user type php_ver vhost site_password; do
    [[ -n "${domain}" ]] || continue
    bootstrap_site_on_standby "${domain}" "${user}" "${type}" "${php_ver}" "${vhost}" "${site_password}"
  done < <(inventory_sites "${db_snap}")

  log_info "Bootstrapping missing databases on standby"
  while IFS='|' read -r site_id domain_name site_user db_name db_user; do
    [[ -n "${db_name}" ]] || continue
    bootstrap_database_on_standby "${domain_name}" "${db_name}" "${db_user}" "${site_user}"
  done < <(inventory_databases "${db_snap}")
}
